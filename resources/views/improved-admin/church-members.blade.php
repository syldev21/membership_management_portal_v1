@extends('layouts.improved-master')
@section('content')
    <div class="side-profile-here">
        ...
        <div>
            ...
            <div style="float: left; width: 75%; height: 38px" class="bar_ups bg-primary text-white"><h2
                    class="mb-4">{{in_array($category_name, config('membership.statuses.cell_group'))?$category_name. ' Cell Group Members':$category_name.' '.($category_detail_description)}}
                    <span
                        class="spanned_status_category display_for_progress text-danger text-warning text-success bg-body"></span>
                    <span class="spanned_conditional_display"></span></h2></div>
            <div style="float: left" class="mr-2 ml-5">
                <div style="float: left">
                    <ul class="navbar-nav ms-auto ms-md-0 me-3 me-lg-4">
                        <li class="nav-item dropdown text-center">
                            <a class="text-black nav-link dropdown-toggle bg-success" id="navbarDropdown" href="#"
                               role="button" data-bs-toggle="dropdown" aria-expanded="false"><span><i
                                        class=""></i></span>More Criteria</a>

                            <ul style="background-color: lightcyan" class="dropdown-menu dropdown-menu-end"
                                aria-labelledby="navbarDropdown">
                                <li>
                                    <a href="#" data-id="" data-user_first_name="" data-user_other_names=""
                                       data-u_name="" data-user_email="" data-user_phone="" id="edit" value=""
                ...
                <div style="float: right">
                    <select name="" id="report_status_category"
                            class="form-select rounded display_for_progress bg-success bg-warning bg-danger bg-info"
                            style="color: white">
                ...
                <input type="hidden" id="member_category_name" data-category="{{$category}}"
                       data-category_name="{{$category_name??null}}"
                       data-member_category="{{$member_age_cluster_category_id ?? null}}">
            </div>
            <input type="hidden"
                   value="{{auth()->user()->hasPermissionTo(config('membership.permissions.Add_Members.text'))}}"
                   id="can-add-members">
            <div style="float: right" id="add_new_one">
                <button class="btn btn-primary" id="edit_modal_button" data-bs-toggle="modal"
                        data-bs-target="#editModal"><span><i class="fa fa-user-plus text-black"></i> </span>Add Member
                </button>
            </div>
        ...
        <diV id="tbldv">
            <div class="table table-responsive m-2" id="main">
                ...
                <table id="dt_select" class="table table-striped table-bordered thead-dark"
                       style="border-top: 1px solid #dddddd; border-bottom: 1px solid #dddddd ">
                    <thead>
                    @if($priv)
                        <tr>
                            <th rowspan="2">S/R</th>
                            ...
                            <th colspan="10" class="permissions-column"
                                style="text-align: center; background-color: #f1f1f1; border: 1px solid #ccc;">
                                Permissions
                            </th>
                    ...
                    @else
                        <tr>
                            ...
                            <th>Sub-County</th>
                            <th>Ward</th>
                            <th class="conditional_show"
                                data-id="{{in_array($category_name, config('membership.statuses.cell_group')) ?? false}}">
                                Cell Group
                            </th>
                    ...
                    <tboby>
                        @foreach($members as $member)
                            <input type="hidden" class="limited_view_and_action"
                                   data-cell_group="{{\App\Models\User::where('id', auth()->id())->with('roles')->first()->cell_group_id ?? null}}"
                                   value="{{\App\Models\User::where('id', auth()->id())->with('roles')->first()->roles[0]->role_id ?? null}}">
                                <?php
...
//                            return response($image->encode('png'));
                            ...
                            <tr class="item{--><!--{$member->id}  text-black fw-bolder}">
                                @if($priv)
                                    ...
                                    <td>
                                        <i class="{{ $member->hasPermissionTO(config('membership.permissions.Add_Members.text')) ? 'fa fa-check' : 'fa fa-x' }}"
                                           style="color: white; background-color: {{ $member->hasPermissionTO(config('membership.permissions.Add_Members.text')) ? 'green' : 'red' }}; border-radius: 50%; padding: 5px;"></i>
                                    </td>
                                    <td>
                                        <i class="{{ $member->hasPermissionTO(config('membership.permissions.Assign_Role.text')) ? 'fa fa-check' : 'fa fa-x' }}"
                                           style="color: white; background-color: {{ $member->hasPermissionTO(config('membership.permissions.Assign_Role.text')) ? 'green' : 'red' }}; border-radius: 50%; padding: 5px;"></i>
                                    </td>
                                    <td>
                                        <i class="{{ $member->hasPermissionTO(config('membership.permissions.Decline_Membership.text')) ? 'fa fa-check' : 'fa fa-x' }}"
                                           style="color: white; background-color: {{ $member->hasPermissionTO(config('membership.permissions.Decline_Membership.text')) ? 'green' : 'red' }}; border-radius: 50%; padding: 5px;"></i>
                                    </td>
                                    <td>
                                        <i class="{{ $member->hasPermissionTO(config('membership.permissions.Delete_Members.text')) ? 'fa fa-check' : 'fa fa-x' }}"
                                           style="color: white; background-color: {{ $member->hasPermissionTO(config('membership.permissions.Delete_Members.text')) ? 'green' : 'red' }}; border-radius: 50%; padding: 5px;"></i>
                                    </td>
                                    <td>
                                        <i class="{{ $member->hasPermissionTO(config('membership.permissions.Edit_Members.text')) ? 'fa fa-check' : 'fa fa-x' }}"
                                           style="color: white; background-color: {{ $member->hasPermissionTO(config('membership.permissions.Edit_Members.text')) ? 'green' : 'red' }}; border-radius: 50%; padding: 5px;"></i>
                                    </td>
                                    <td>
                                        <i class="{{ $member->hasPermissionTO(config('membership.permissions.prepare_registration.text')) ? 'fa fa-check' : 'fa fa-x' }}"
                                           style="color: white; background-color: {{ $member->hasPermissionTO(config('membership.permissions.prepare_registration.text')) ? 'green' : 'red' }}; border-radius: 50%; padding: 5px;"></i>
                                    </td>
                                    <td>
                                        <i class="{{ $member->hasPermissionTO(config('membership.permissions.review_registration.text')) ? 'fa fa-check' : 'fa fa-x' }}"
                                           style="color: white; background-color: {{ $member->hasPermissionTO(config('membership.permissions.review_registration.text')) ? 'green' : 'red' }}; border-radius: 50%; padding: 5px;"></i>
                                    </td>
                                    <td>
                                        <i class="{{ $member->hasPermissionTO(config('membership.permissions.approve_registration.text')) ? 'fa fa-check' : 'fa fa-x' }}"
                                           style="color: white; background-color: {{ $member->hasPermissionTO(config('membership.permissions.approve_registration.text')) ? 'green' : 'red' }}; border-radius: 50%; padding: 5px;"></i>
                                    </td>
                                    <td>
                                        <i class="{{ $member->hasPermissionTO(config('membership.permissions.See_Members.text')) ? 'fa fa-check' : 'fa fa-x' }}"
                                           style="color: white; background-color: {{ $member->hasPermissionTO(config('membership.permissions.See_Members.text')) ? 'green' : 'red' }}; border-radius: 50%; padding: 5px;"></i>
                                    </td>
                                    <td>
                                        <i class="{{ $member->hasPermissionTO(config('membership.permissions.generate_report.text')) ? 'fa fa-check' : 'fa fa-x' }}"
                                           style="color: white; background-color: {{ $member->hasPermissionTO(config('membership.permissions.generate_report.text')) ? 'green' : 'red' }}; border-radius: 50%; padding: 5px;"></i>
                                    </td>
                                @else
                                    <td>{{$loop->iteration}}</td>
                                    <td>{{$member->member_number??''}}</td>
                                    ...
                                    <td class="conditional_show"
                                        data-id="{{in_array($category_name, config('membership.statuses.cell_group')) ?? false}}">{{isset($member->cell_group_id)?config('membership.statuses.cell_group')[$member->cell_group_id]:''}}</td>
            ...
            //                            @endphp
                                ...
                                <td class="hide_for_execs">
                                    <ul class="navbar-nav ms-auto ms-md-0 me-3 me-lg-4">
                                        <li class="nav-item dropdown text-center">
                                            <a class="text-black nav-link dropdown-toggle" id="navbarDropdown" href="#"
                                               role="button" data-bs-toggle="dropdown" aria-expanded="false"><span><i
                                            ...
                                            <ul style="background-color: lightcyan"
                                                ...
                                                <li>
                                                    <a href="#" data-id="{{$member->id}}"
                                                       data-user_first_name="{{explode(' ', $member->name)[0]}}"
                                                       data-user_other_names="{{isset($member->name)?implode(' ', array_slice(explode(' ', $member->name), 1)):''}}"
                                                       data-u_name="{{$member->user_name}}"
                                                       data-user_email="{{$member->email}}"
                                                       data-user_phone="{{$member->phone}}" id="edit"
                                                       value="{{$member->id}}" class="dropdown-item text-primary"
                                                ...
                                                <li>
                                                    <a href="#" data-id="{{$member->id}}"
                                                       data-existing="{{$member->existing}}"
                                                       data-hide="{{auth()->id()}}"
                                                       data-user_first_name="{{explode(' ', $member->name)[0]}}"
                                                       data-user_name="{{$member->name}}" id="delete" value=""
                                                       class="dropdown-item hide-this text-danger"
                                                ...
                                                <li>
                                                    <a href="#" data-id="{{$member->id}}" data-hide="{{auth()->id()}}"
                                                       data-user_name="{{$member->name}}"
                                                       data-activate="{{$member->active}}" id="deactivate"
                                                       value="{{$member->id}}"
                                                       class="dropdown-item user_status  hide-this text-warning"
                                                ...
                                                @if(auth()->user()->hasPermissionTo(config('membership.permissions.Assign_Role.text')))
                                                    <li>
                                                        <a href="#" data-id="{{$member->id}}"
                                                           data-user_title="{{$member->name}}"
                                                           data-user_name="{{$member->name}}"
                                                           data-user_email="{{$member->email}}"
                                                           data-user_phone="{{$member->phone}}" id="role"
                                                           value="{{$member->id}}" class="dropdown-item text-success"
                                                ...
                                                <li>
                                                    <a href="#" class="dropdown-item text-secondary"
                                                       data-cell_group="{{config('membership.statuses.cell_group')[$member->cell_group_id]??null}}"
                                                       data-user_first_name="{{explode(' ', $member->name)[0]}}"
                                                       data-user_name="{{$member->name}}" data-id="{{$member->id}}"
                                                       data-bs-toggle="modal" data-bs-target="#reviewModal"
                                                       data-registration_status="{{$member->registration_status}}"
                                                       id="reviewRegistration">
        ...
        <div class="modal fade table table-responsive" id="roleModal" tabindex="-1" aria-labelledby="exampleModalLabel"
             aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                                ...
                                <table class="table table-responsive table-bordered table-striped thread-dark">
                                    ...
                                    <tr>
                                        ...
                                        <th rowspan="2" class="align-content-center text-center">
                                            <button id="clear_role_button" type="button" class="btn btn-warning">Clear
                                                Selection
                                            </button>
                                        </th>
                                    </tr>
                    ...
                    <div class="modal-footer d-flex justify-content-between">
                        ...
                        <input type="button" class="btn btn-primary" id="assign_role_button" data-bs-popper=""
                               value="Assign">
                    </div>
                </div>
            </div>
        ...
        <div class="modal fade table table-responsive" id="editModal" tabindex="-1" aria-labelledby="exampleModalLabel"
             aria-hidden="true">
                        ...
                        <div class="col-lg-12">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-12 px-5">
                                    <form action="#" method="POST" id="profile_edit_form"
                                          class="accordion-flush admin_edit">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg">
                                                ...
                                                <input type="text" name="otherNames" class="form-control rounded-0 "
                                                       id="otherNames" value="">
                                                <div class="invalid-feedback"></div>
                                            </div>
                                            <div class="col-lg">
                                                <label class="fw-bold" for="name">Email <span class="spanned_or_phone">or Phone</span></label>
                                                <input type="text" name="unique_id" class="form-control rounded-0 "
                                                       id="unique_id" value="">
                                                <div class="invalid-feedback"></div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            ...
                                            <div class="col-lg">
                                                ...
                                                <input data-toggle="tooltip" data-placement="bottom"
                                                       title="The year you joined VOSH Church Int'l not necessarily Buru Buru!"
                                                       type="date" name="year_joined"
                                                       class="form-control rounded-0 profile_edit" id="year_joined"
                                                       value="">
                                                <div class="invalid-feedback"></div>
                                            </div>
                                            <div class="col-lg">
                                                <label class="fw-bold" for="dob" class="">Date of Birth</label>
                                                <input type="date" name="dob" class="form-control rounded-0 " id="dob"
                                                       value="">
                                                <div class="invalid-feedback"></div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            ...
                                            <div class="col-lg">
                                                <label class="fw-bold hide-status" for="marital_status">Marital
                                                    Status</label>
                                                <select name="marital_status" id="marital_status"
                                                        class="form-select rounded-0  hide-status hide-field">
                                                    <option selected value="">--Select--</option>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg">
                                                ...
                                                <select name="cell_group" id="cell_group"
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg">
                                                ...
                                                <select name="born_again" id="born_again"
                                                        class="form-select rounded-0 ">
                                                    <option selected value="">--Select--</option>
                                        ...
                                        <div class="row">
                                            <div class="col-lg">
                                                ...
                                                <select name="leadership_status" id="leadership_status"
                                                        class="form-select rounded-0 ">
                                                    <option selected value="">--Select--</option>
                                                    @foreach(config('membership.flag') as $flag)
                                                        <option value="{{$flag['id']}}">
                                        ...
                                        <div class="row">`
                                            <label class="fw-bold" for="leadership_status">Other Ministries/ Ministries
                                                of Interest (Tick as appropriate)</label>
                                            @foreach(config('membership.ministry') as $ministry)
                                                <div class="form-check col-lg">
                                                    <input type="checkbox" class="check_box" id="check_box"
                                                           name="check_box[]" value="{{$ministry['id']}}">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg">
                                                <label class="fw-bold hide-status" for="employment_status">Employment
                                                    Status</label>
                    ...
                    <div class="modal-body">
                        <form method="" id="">
                            <div class="my-2 delete_data">
                                ...
                                <select name="delete_data" id="delete_data"
                                        class="form-select rounded-0  hide-status hide-field">
                                    <option selected value="">--Select--</option>
                            ...
                            <div class="show-for-declining">
                                ...
                                <div class="my-2">
                                    @if (isset($member) && $member->registration_status ==5)
                                        <label class="fw-bold hide-status condition_decline_reason_title"
                                               for="decline_data">Reason for Approval</label>

                                        <select name="decline_data" id="decline_data"
                                                class="form-select rounded-0  hide-status hide-field">
                    ...
                    <div class="modal-body">
                        <form method="" id="">
                            ...
                            <div class="my-2 hide-delete-field">
                                ...
                                <div class="invalid-feedback"></div>
                                <select name="deactivate_data" id="deactivate_data"
                                        class="form-select rounded-0  hide-status hide-field">
                                    <option selected value="">--Select--</option>
